...
中小型 × 新規(初回) 大量保有報告書 自動ツイート（1回の実行で最もインパクトのある1件のみ）
  ...
  - 選ばれなかった候補は tweeted に積まないので、後続の実行で順に拾われる#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
中小型 × 新規(初回) 大量保有報告書 自動ツイート（1回の実行で最もインパクトのある1件のみ）
...
def is_notable(r: dict, notable: list) -> bool:
    """提出者が有名ファンド(notable_filers.txt のいずれか)に部分一致するか。"""
    return any(k in (r.get("filer") or "") for k in notable)
...
def main():
    ...
    cands = [r for r in reports
             if "smallmid" in r["universe"] and r["isNew"] and r["docID"] not in seen]
    log(f"候補(中小型×新規×未投稿): {len(cands)} 件")
